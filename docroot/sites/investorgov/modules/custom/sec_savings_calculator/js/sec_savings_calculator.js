...
(function ($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.savingsCalculatorResetBehavior = {
    attach: function (context, settings) {
      if ($('.messages--error').length > 0) {
        $('#savings-goal-calc', context).once('calculator_error').each(function () {
      ...
      $('#savings-goal-calc', context).once('calculator_reset').each(function () {
        ...
        $('.button--reset').click(function (event) {
          ...
          $('#savings-goal-calc').find('input:first').focus();
        });
        ...
        var resetCalculator = function () {
          ...
          drupalSettings.sec_savings_calculator = {};
        }
  ...
  Drupal.behaviors.savings_calculator = {
    attach: function (context, settings) {
      $('#savings-goal-calc', context).once('sec_savings_calculator').each(function () {

        $(document).ajaxComplete(function (event, xhr, settings) {
          if (!$.isEmptyObject(drupalSettings.sec_savings_calculator)) {
            var messageBox = $('.page__title').next('div');
            $('#compound-interest-calc, #savings-goal-calc').focus();
            $('#calculator_results_chart').empty();
            $('.highcharts-data-table').remove();
            var mySeries = [];
            var calcType = drupalSettings.sec_savings_calculator.calcType;
            var calcYears = drupalSettings.sec_savings_calculator.calcYears;

            switch (calcType) {
              case 'basic':
                var interestRateBase = drupalSettings.sec_savings_calculator.interestRateBase;
                var interestRatePlus = drupalSettings.sec_savings_calculator.interestRatePlus;
                var interestRateMinus = drupalSettings.sec_savings_calculator.interestRateMinus;
                var calcResults = {
                  name: 'Future Value (' + (interestRateBase * 100).toFixed(2) + '%)',
                  data: drupalSettings.sec_savings_calculator.calcResults,
                  color: 'rgba(191,40,13,1)',
                };
                if (drupalSettings.sec_savings_calculator.hasVar) {
                  var calcMinus = {
                    name: 'Variance Below (' + (interestRateMinus * 100).toFixed(2) + '%)',
                    data: drupalSettings.sec_savings_calculator.calcResultsMinus,
                    color: 'rgba(38,144,146,1)',
                  };
                  var calcPlus = {
                    name: 'Variance Above (' + (interestRatePlus * 100).toFixed(2) + '%)',
                    data: drupalSettings.sec_savings_calculator.calcResultsPlus,
                  ...
                  var calcDataBase = {
                    ...
                    data: drupalSettings.sec_savings_calculator.calcResultsBase,
                    color: 'rgba(38,144,146,1)',
                  };
              ...
              case 'monthly':
                ...
                var calcBase = {
                  ...
                  data: drupalSettings.sec_savings_calculator.baseResults,
                  color: 'rgba(0,50,91,1)',
                };
                var calcMonthly = {
                  name: "Total Monthly Additions To-Date",
                  data: drupalSettings.sec_savings_calculator.monthlyResults,
                  color: 'rgba(191,40,13,1)',
                };
            ...
            var createResultTable = function (data) {
              ...
              for (i = 0; i < rows; i++) {
                ...
                for (j = 0; j < columns; j++) {
                  ...
                  newTable += '<td class="' + classNames + '" >' + data[j].data[i].toLocaleString('en-US', { style: 'currency', currency: 'USD' }) + '</td>';
                }
                newTable += '</tr>'
              }
              newTable += '</tbody>';

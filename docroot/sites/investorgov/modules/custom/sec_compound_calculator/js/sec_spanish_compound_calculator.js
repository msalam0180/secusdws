/**
 * @file
 * Contains Javascript for Spanish Compound Interest Calculator.
 */
    ...
    attach: function (context, settings) {
      $('#compound-interest-calc', context).once('calculator_reset').each(function () {
        ...
        $('.button--reset').click(function (event) {
          ...
          $('#compound-interest-calc').find('input:first').focus();
        });
        ...
        $(document).ajaxComplete(function (event, xhr, settings) {
          ...
          if (!$.isEmptyObject(drupalSettings.sec_compound_calculator)) {
            ...
            switch (calcType) {
              case 'basic':
                ...
                var calcResults = {
                  name: 'Valor futuro (' + (interestRateBase * 100).toFixed(2) + '%)',
                ...
                if (drupalSettings.sec_compound_calculator.hasVar) {
                  var calcMinus = {
                    name: 'Varianza inferior (' + (interestRateMinus * 100).toFixed(2) + '%)',
                    data: drupalSettings.sec_compound_calculator.calcResultsMinus,
                    color: 'rgba(38,144,146,1)',
                  };
                  var calcPlus = {
                    name: 'Varianza superior (' + (interestRatePlus * 100).toFixed(2) + '%)',
              ...
              case 'monthly':
                var calcResults = {
                  name: "Ahorro total compuesto",
                  data: drupalSettings.sec_compound_calculator.calcResults,
                  color: 'rgba(38,144,146,1)',
                };
                var calcBase = {
                  name: "Inversión inicial compuesta",
                  data: drupalSettings.sec_compound_calculator.baseResults,
                  color: 'rgba(0,50,91,1)',
                };
                var calcMonthly = {
                  name: "Total de adiciones mensuales hasta la fecha",
            ...
            var myChart = Highcharts.chart('calculator_results_chart', {
              chart: {
                type: 'line',
                description: "Tasa de interés acumulada durante " + (calcYears.length - 1) + " años."
              },
              tooltip: {
                shared: true,
                valuePrefix: '$',
              ...
              exporting: {
                showTable: false,
                tableCaption: "Ahorros totales en dólares estadounidenses"
              }
            });
            drupalSettings.sec_compound_calculator = {};
          }
          ...
          var createResultTable = function (data) {
            if (data) {
              ...
              newTable += '<caption class="highcharts-table-caption">Ahorros totales en dólares estadounidenses</caption>';
              newTable += '<thead><tr>';
              newTable += '<th scope="col" class="text">Años</th>';

              for (y = 0; y < columns; y++) {
                var classNames = (data[y].name === 'Future Value' || data[y].name === 'Ahorros totales en dólares estadounidenses') ? ('text ' + highlighted) : ('text');
              ...
              for (i = 0; i < rows; i++) {
                newTable += '<tr>';
                newTable += '<td scope="row" class="text">Año ' + i + '</td>';
                for (j = 0; j < columns; j++) {

...
class EntityContext extends RawDrupalContext {
  ...
  public function iCreateMenuLinkContent(TableNode $table) {
    ...
    foreach($table_hash as $link_hash) {
      ...
      if (!empty($link_hash['parent_uri']) && !empty($link_hash['parent_title'])) {
        $query = \Drupal::service('entity_type.manager')->getStorage('menu_link_content')->getQuery()
        ...
        if (!empty($result)) {
          ...
          $parent_menu_link = \Drupal::service('entity_type.manager')->getStorage('menu_link_content')->load($parent_id);
  ...
  public function createEntities($entity_type, $bundle, $hash) {
    $saved = [];
    ...
    foreach ($hash as $entity_hash) {
	  ...
	  if (isset($entity_hash['field_start_date'])) {
        $date = new \DateTime($entity_hash['field_start_date'], new \DateTimeZone("UTC"));
        $entity_hash['field_start_date'] = $date->format('Y-m-d\TH:i:s');
      }

      if (isset($entity_hash['field_end_date'])) {
        $date = new \DateTime($entity_hash['field_end_date'], new \DateTimeZone("UTC"));
  ...
  public function cleanEntities() {
    foreach ($this->entities as $entity_type => $entities) {
      ...
      $storage_handler = \Drupal::service('entity_type.manager')->getStorage($entity_type);
